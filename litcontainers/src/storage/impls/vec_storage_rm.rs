...
impl<T, R, C> InplaceMapOrdered<T> for VecStorageRM<T, R, C>
	where T: Element, R: Dim, C: Dim
{
	fn map_inplace_ordered<F: FnMut(&mut T)>(&mut self, f: F) { self.map_inplace(f) }
}

impl<T, R, C> InplaceForeach<T> for VecStorageRM<T, R, C>
	where T: Element, R: Dim, C: Dim
{
