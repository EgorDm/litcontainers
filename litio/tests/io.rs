...
fn test_binary_read_write() {
	...
	let s2: ContainerRM<f64, U2, U2> = BinarySerializer::read(&tmp[..]).unwrap();
...
fn test_to_file() {
	...
	let s1 = ContainerRM::regspace_rows(U2, U2, 0.);
	litio::write_binary(in_path.as_path(), &s1).unwrap();
	let s2: ContainerRM<f64, U2, U2> = litio::read_binary(in_path.as_path()).unwrap();
